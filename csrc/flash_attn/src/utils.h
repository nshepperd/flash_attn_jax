...
__forceinline__ __device__ void relu_(cute::Tensor<Engine, Layout> &tensor) {
    ...
    // HACK: this requires tensor to be "contiguous"
    cute::Tensor tensor_uint32 = recast<uint32_t>(tensor);
    #pragma unroll
...
#else
    cute::Tensor out = FLASH_NAMESPACE::convert_type<To_type>(tensor);
...
template <typename Engine, typename Layout>
__forceinline__ __device__ void apply_softcap(cute::Tensor<Engine, Layout> &tensor, const float softcap){
    #pragma unroll
...
__forceinline__ __device__ void calculate_dtanh(cute::Tensor<Engine0, Layout0> &src_tensor, cute::Tensor<Engine1, Layout1> &dst_tensor, const float softcap){
    #pragma unroll
