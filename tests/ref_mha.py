...
def make_mask(R, C, is_causal, window_size):
    # mask is "left-padded" with 1s when R<C; a shorter q sees all of k
...
def ref_fwd(q,k,v, is_causal=False, window_size=(-1,-1), softmax_scale=None):
    ...
    [n, lk, hk, d] = k.shape
    if h != hk:
        q = rearrange(q, "n l (h x) d -> x n l h d", h=hk)
        out = jax.vmap(
            partial(
                ref_fwd,
                is_causal=is_causal,
        ...
        o = rearrange(out[0], "x n l h d -> n l (h x) d")
        lse = rearrange(out[1], "x n h l -> n (h x) l")
        return o, lse

    if softmax_scale is None:
        softmax_scale = 1/np.sqrt(d)
    mask = make_mask(l,lk,is_causal,window_size)
    q_mask = jnp.any(mask, axis=1)
    ...
    o = jnp.where(q_mask[None,:,None,None], o, 0.0)
